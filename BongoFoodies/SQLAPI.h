...
enum eSAClient
{
	...
	SA_DuckDB_Client, //!< DuckDB client
...
protected:
	static bool DateFromTm(
		...
		unsigned int nNanoSecond, bool hasDate,
		double& dtDest);
...
public:
	...
	SADateTime& operator=(const SADateTime& dtSrc);
};
...
public:
	...
	int getOptionValue(const SAString& sOption, int nDefault) const;
	SAString getOptionValue(const SAString& sOption, const SAChar* szDefault) const;
...
class SQLAPI_API SAValueRead
{
	...
	friend class IssOleDbSequentialStream;
	friend class IduckdbCursor;
